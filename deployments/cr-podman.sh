...
podman run --pod zuul --name zuul-executor --privileged --detach --volume=zuul-zuul:/etc/zuul --volume=executor-ssh-key:/etc/zuul-executor --volume=rdo-key:/etc/zuul-gerrit-review.rdoproject.org --rm quay.io/software-factory/zuul-executor:3.4 "zuul-executor" "-d"
...
podman run --pod zuul --name zuul-scheduler --detach --volume=zuul-zuul:/etc/zuul --volume=zuul-yaml-conf:/etc/zuul-scheduler --volume=rdo-key:/etc/zuul-gerrit-review.rdoproject.org --rm quay.io/software-factory/zuul-scheduler:3.4 "zuul-scheduler" "-d"
