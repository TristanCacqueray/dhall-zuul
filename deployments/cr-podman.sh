...
podman run --pod zuul --name zuul-launcher --detach --volume=zuul-nodepool:/etc/nodepool --volume=nodepool-yaml-conf:/etc/nodepool-user --volume=kube-config:/etc/nodepool-kubernetes/ --env=KUBECONFIG='/etc/nodepool-kubernetes/kube.config' --env=OS_CLIENT_CONFIG_FILE='/etc/nodepool-openstack/undefined' --rm quay.io/software-factory/nodepool-launcher:3.4 "sh" "-c" "cat /etc/nodepool/nodepool.yaml /etc/nodepool-user/nodepool.yaml > ~/nodepool.yaml ; nodepool-launcher -d -c ~/nodepool.yaml"
